...
/* Names are generated automatically so nothing blocks the first PLAY, but
   the field on the main menu (#playerName) lets the player change it —
   that is the only place a name is ever typed, and it saves as you type. */
function initFirstPlayerName() {
  ...
  let cleaned = typeof NETP !== 'undefined' ? NETP.cleanPlayerName(saved) : saved.trim().slice(0, 20);
  if (!cleaned) {
    const auto = 'Player' + Math.floor(1000 + Math.random() * 9000);
    cleaned = typeof NETP !== 'undefined' ? (NETP.cleanPlayerName(auto) || auto) : auto;
  }
  ...
  if (input) {
    ...
    input.addEventListener('input', () => {
      const c = typeof NETP !== 'undefined' ? NETP.cleanPlayerName(input.value) : input.value.slice(0, 20);
      if (input.value !== c) input.value = c;
      if (c) { try { localStorage.setItem('pastel-nuketown-name', c); } catch (e) {} }
    });
    input.addEventListener('blur', () => {
      if (!input.value) {
        let fallback = '';
        try { fallback = localStorage.getItem('pastel-nuketown-name') || ''; } catch (e) {}
        input.value = fallback || cleaned;
      }
    });
  }
}
...
/* Kept in step with G.botsEnabled the same way the DM/KC buttons track
   G.mode: read the live state here rather than trust whichever button was
   last clicked. Hidden for guests — bots are a host decision, same as the
   mode and map are. */
function syncPlayersPicker() {
  const box = document.getElementById('playersOptions');
  const bots = document.getElementById('modeBots');
  const playersOnly = document.getElementById('modePlayersOnly');
  const label = document.getElementById('playersPickerLabel');
  if (!box || !bots || !playersOnly) return;

  const guest = typeof netIsGuest === 'function' && netIsGuest();
  box.hidden = guest;
  if (label) label.hidden = guest;
  if (guest) return;

  const on = !!G.botsEnabled;
  bots.setAttribute('aria-pressed', String(on));
  playersOnly.setAttribute('aria-pressed', String(!on));
}

function choosePlayersMode(enabled) {
  if (typeof netIsGuest === 'function' && netIsGuest()) return;
...
function boot() {
  ...
  document.getElementById('modeBots').addEventListener('click', () => choosePlayersMode(true));
  document.getElementById('modePlayersOnly').addEventListener('click', () => choosePlayersMode(false));
