...
function netBotCount(humanCount) {
  ...
  if (netIsHost()) {
    if (!G.botsEnabled) return 0;
    return Math.max(0, CFG.combatants - humanCount);
  }
  return CFG.bots;
}
...
function netConnect(kind, requestedRoom, quickPlan) {
  ...
  /* No UI asks for a name anywhere — it is generated once and stored by
     initFirstPlayerName(), and this just reads it back. */
  let savedName = ''; try { savedName = localStorage.getItem('pastel-nuketown-name') || ''; } catch (e) {}
  let name = NETP.cleanPlayerName(savedName);
  if (!name) { name = NETP.cleanPlayerName('Player' + Math.floor(1000 + Math.random() * 9000)) || 'Player'; }
  ...
  const socketURL = netWsURL();
  if (!socketURL) {
    console.error('[nuketown] netConnect: could not resolve a relay URL', { requested: QS.get('server') });
    netResetTransport();
    netShowMainMenu();
  ...
  try { ws = new WebSocket(socketURL); }
    ...
    console.error('[nuketown] netConnect: WebSocket construction failed for', socketURL, e);
    netResetTransport();
    netShowMainMenu();
  ...
  ws.addEventListener('error', (e) => {
    console.error('[nuketown] WebSocket error while connecting to', socketURL, e);
  ...
  ws.addEventListener('close', (e) => {
    ...
    if (!manual) {
      console.error('[nuketown] WebSocket closed unexpectedly', {
        url: socketURL, code: e.code, reason: e.reason, wasClean: e.wasClean, phase: NET.phase
      });
    }
    NET.socket = null;
    if (!manual) {
  ...
  NET.connectTimer = setTimeout(() => {
    if (NET.phase !== 'connecting') return;
    console.error('[nuketown] netConnect: no response from relay within 9s', socketURL);
...
function netHandleWire(raw) {
  let msg;
  try { msg = JSON.parse(raw); } catch (e) { console.error('[nuketown] netHandleWire: malformed message from relay', raw, e); return; }
...
function netHostStart() {
  ...
  // PLAYERS ONLY has no bots to make up the numbers, so it needs a second
  // real player in the room before there is anyone to actually play against.
  if (!G.botsEnabled && NET.members.length < 2) {
    netStatus('برای «پلیر» حداقل به ۲ بازیکن نیاز است.', 'error');
    return;
  }
  NET.starting = true;
  netArmStartTimeout();
...
function netBeginMatch() {
  ...
  if (netIsMultiplayer()) {
    /* IMPORTANT: the start packet itself is the shared match-start barrier.
       Do not pause guests here. The old implementation deliberately opened
       the pause/deploy card because Pointer Lock cannot be requested from a
       WebSocket callback (it is not a browser user gesture). That made the
       guest simulation stop immediately after the host started the round,
       while the host kept running at 60 Hz. Besides forcing a click, it also
       made the guest appear to have a huge network delay because its local
       prediction was frozen behind G.paused.

       The match must start on the same start packet for every member. Pointer
       Lock is only an input affordance: if the browser refuses the automatic
       request, the existing canvas mousedown handler will acquire it on the
       player's first click without stopping the simulation. */
    setPaused(false);
    netSetMenuBusy(false);
    document.getElementById('play').textContent = 'چند نفره';
    const note = document.getElementById('menuNote');
...
function netBackfillBots() {
  /* PLAYERS ONLY is a hard room invariant. A player leaving must free a seat,
     not cause the host to manufacture an AI replacement. The old function was
     called from the disconnect/prune path without checking G.botsEnabled, so
     a players-only match silently changed rules as soon as somebody left. */
  if (!netIsHost() || NET.phase !== 'playing' || G.over || !G.botsEnabled) return;
